...
Local aTotICMSST	:= {} // mantido por compatibilizacao - nao e mais utilizado
...
Local aRetIcms      := {}
local aBenef		:= {}				   
//TBC-GO Crio Array para complemento mensagem monofasica
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				//TBC-GO
				If lNfCup .And. SM0->M0_ESTCOB $ "CE/DF/MG/TO/MA" //-- "CE/DF" 
					lNfCupZero	:= .T.
					aAreaSF2  	:= SF2->(GetArea())
					DbSelectArea( "SF2" )
					DbSetOrder(1)  // F2_DOC + F2_SERIE + F2_CLIENTE + F2_LOJA
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If lNfCup
							...
							//TBC-GO Adicionado +(cAliasSD2)->D2_COD+cNumitem
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "STMONO"
									aTail(aICMSMono) := {CD2->CD2_ORIGEM,;	//TBC-GO Alterado a logica de validacao para carregar valores quando usar o Tributador de: lNfCupZero para:(lNfCupZero .and. CD2->CD2_CST <> '61')
													   If((lNfCupZero .and. CD2->CD2_CST <> '61'),SF4->F4_SITTRIB,CD2->CD2_CST),;
													   CD2->CD2_MODBC,;
									                   If((lNfCupZero .and. CD2->CD2_CST <> '61'),0,nMargem),;
													   If( (lNfCupZero .and. CD2->CD2_CST <> '61') .Or. lIcmsPR,0,CD2->CD2_BC),;
									If((lNfCupZero .and. CD2->CD2_CST <> '61') ,0 , Iif(CD2->CD2_BC>0,xFisRetFCP('4.0','CD2','CD2_ALIQ'),0) ),;
									If((lNfCupZero .and. CD2->CD2_CST <> '61') .Or. lIcmsPR,0,nValtrib),;
									0,;
									CD2->CD2_QTRIB,;
									CD2->CD2_PAUTA,;
									SFT->FT_MOTICMS,;
...
IF lPe01Nfe     
	...
	//-- TBC - GO Acrescentado o vetor aComb para manipular os dados complementares de combustiveis, e outras variaveis e arrays
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aProcRef,aMed,aLote,;
...
If !Empty(aNota)
	...
	if lMonof61
		...
		//TBC-GO Complemento mensagem monofasico
...
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2', ComplPreco(cTipo,cF2Tipo,aProd), IIf(!lZeraValTag, ConvType(Round(aProd[10]/aProd[12],10),23,10), ConvType(nZeraTag,23,10)))+'</vUnTrib>'
...
If  !lIssQn    
	...
	ElseIf aCST[01] $ "02-15-53-61"
		...
		If(aCST[1] $ '61')
			...
			//TBC-GO tratativa para complemento da mensagem monofasico
			if len(aImpMono) >= 7
...
IF len(aDetPag) > 0  
	...
	For nX := 1 To Len(aDetPag)
		...
		//TBC-GO - Rejeição 441: Descrição do pagamento obrigatória para meio de pagamento 99- outros
