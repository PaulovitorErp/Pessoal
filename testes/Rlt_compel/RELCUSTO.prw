...
SELECT
    ...
    CASE
        ...
        WHEN CTT.CTT_XCD = 'D' AND SF4.F4_ESTOQUE = 'N' THEN LTRIM(RTRIM(SB1.B1_XCTAD)) + '-' + LTRIM(RTRIM(CT1DESP.CT1_DESC01))
        WHEN CTT.CTT_XCD = 'C' AND SF4.F4_ESTOQUE = 'N' THEN LTRIM(RTRIM(SB1.B1_XCTAC)) + '-' + LTRIM(RTRIM(CT1CUSTO.CT1_DESC01))
...
INNER JOIN (
    SELECT DISTINCT F4_CODIGO, F4_ESTOQUE, F4_ATUATF FROM %table:SF4%
) SF4 ON SD1.D1_TES = SF4.F4_CODIGO
LEFT JOIN (
