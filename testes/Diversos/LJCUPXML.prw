                ...
                if CD2->(DbSeek(cChaveCD2))
                    ...
                    if nAliq > 0
                        ...
                        if SF3->(DbSeek(xFilial("SF3") + SD2->D2_CLIENTE + SD2->D2_LOJA + SD2->D2_DOC + SD2->D2_SERIE))
                                ...
                                While SF3->(!Eof()) ;
                                    .AND. SF3->F3_FILIAL == SF3->F3_FILIAL ;
                                    .AND. SF3->F3_NFISCAL == SD2->D2_DOC ;
                                    .AND. SF3->F3_SERIE == SD2->D2_SERIE ;
                                    .AND. SF3->F3_CLIEFOR == SD2->D2_CLIENTE ;
                                    .AND. SF3->F3_LOJA == SD2->D2_LOJA
                                    // se o mes da nota for diferente do mes da database
                                    if Month(SF3->F3_ENTRADA) <> Month(dDataBase)
                                    ...
                                    SF3->(DbSkip())
                                EndDo
                            SFT->(DbSetOrder(10)) // FT_FILIAL+FT_NFISCAL+FT_SERIE+FT_CLIEFOR+FT_LOJA
                            if SFT->(DbSeek(xFilial("SFT") + SD2->D2_CLIENTE + SD2->D2_LOJA + SD2->D2_DOC + SD2->D2_SERIE ))

                                While SFT->(!Eof()) ;
                                    .AND. SFT->FT_FILIAL == SF3->F3_FILIAL ;
                                    ...
                                    // se o mes da nota for diferente do mes da database
                                    if Month(SFT->FT_ENTRADA) <> Month(dDataBase)
