...
User Function EGPER030(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
...
cBaseAux   := "S" 	//If(mv_par20 == 1,"S","N")							//Imprimir Bases
nDuas      := 2 	//If(mv_par21 == "N",1,2)							//quantas vezes é para imprimir o mesmo contra-Cheque
...
If nTipRel == 3
	...
	Private _oFont13  := TFont():New("Arial", 09, 13,.T.,.T., 5,.T., 5, .T., .F.)
	...
	//Private _oPrint:SetPaperSize(9)   // Define Impressao folha A4
	_oPrint:SetPaperSize(9)   // Define Impressao folha A4
	Private _cont     := 3500
	Private  lFirst   := .t.
	Private _pag      := 0
	Private _cFig     := "\system\LogoInfinIT.png" //MUDAR LOGO CONFORME O LOGO DO CLIENTE
//\system\logo_aviso.jpg
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	IF !( lTerminal )
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If (SRA->RA_CHAPA < ChapaDe) .Or. (SRA->Ra_CHAPa > ChapaAte) .Or. (SRA->RA_NOME < cNomDe) .Or. (SRA->Ra_NOME > cNomAte).Or. ;
		   (SRA->RA_MAT < cMatDe) .Or. (SRA->Ra_MAT > cMatAte) .Or. (SRA->RA_CC < cCcDe) .Or. (SRA->Ra_CC > cCcAte)
			SRA->(dbSkip(1))
			Loop
		EndIf
...
//			Endif
			...
			DbSelectArea("SRX")
			SRX->(dbSetOrder(1))
			SRX->(DbSeek(xFilial("SRX")+"06"+xFilial("SRA")+space(08)+MENSAG1+space(08)))
			DESC_MSG1 := LEFT(SRX->RX_TXT,30)
			SRX->(DbCloseArea())
...
//			Endif
			...
			DbSelectArea("SRX")
			SRX->(dbSetOrder(1))
			SRX->(DbSeek(xFilial("SRX")+"06"+xFilial("SRA")+space(08)+MENSAG2+space(08)))
			DESC_MSG2 := LEFT(SRX->RX_TXT,30)
			SRX->(DbCloseArea())
		Endif
...
//			Endif
			...
			DbSelectArea("SRX")
			SRX->(dbSetOrder(1))
			SRX->(DbSeek(xFilial("SRX")+"06"+xFilial("SRA")+space(08)+MENSAG3+space(08)))
			DESC_MSG3 := LEFT(SRX->RX_TXT,30)
			SRX->(DbCloseArea())
...
//				Endif
		...
		Else
				...
				DbSelectArea("SRD")
				SRD->(DbSetOrder(1))
				If SRD->(DbSeek(xFilial("SRD")+SRA->RA_MAT+Substr(cMesAnoRef,3,4)+Substr(cMesAnoRef,1,2)))
					
				 	cNroHoras   	:= &("{ || If(SRD->RD_QTDSEM > 0, SRD->RD_QTDSEM, SRD->RD_HORAS) }")
					
					While !Eof() .And. SRD->RD_FILIAL+SRD->RD_MAT == SRA->RA_FILIAL+SRA->RA_MAT; 
						.AND. SRD->RD_DATARQ  == Substr(cMesAnoRef,3,4)+Substr(cMesAnoRef,1,2)
					
						if cRotFol == SRD->RD_ROTEIR
...
//							Endif
							...
							If (Esc == 1) .And. (Srd->Rd_Pd == aCodFol[7,1])      // Desconto de Adto
								fSomaPdRec("P",aCodFol[6,1],Eval(cNroHoras),SRD->RD_VALOR,SRD->RD_ROTEIR)
								TOTVENC += Srd->Rd_Valor
							Elseif (Esc == 1) .And. (Srd->Rd_Pd == aCodFol[12,1])
								fSomaPdRec("D",aCodFol[9,1],Eval(cNroHoras),SRD->RD_VALOR,SRD->RD_ROTEIR)
								TOTDESC += SRD->RD_VALOR
							Elseif (Esc == 1) .And. (Srd->Rd_Pd == aCodFol[8,1])
								fSomaPdRec("P",aCodFol[8,1],Eval(cNroHoras),SRD->RD_VALOR,SRD->RD_ROTEIR)
								TOTVENC += SRD->RD_VALOR
							Else
								If PosSrv( Srd->Rd_Pd , SRA->RA_FILIAL , "RV_TIPOCOD" ) == "1"
									If (Esc # 1) .Or. (Esc == 1 .And. SRV->RV_ADIANTA == "S")
										...
										Else
											nHoras := Eval(cNroHoras)
										Endif
										fSomaPdRec("P",SRD->RD_PD,nHoras,SRD->RD_VALOR,SRD->RD_ROTEIR)
										TOTVENC += Srd->Rd_Valor
									Endif
								Elseif SRV->RV_TIPOCOD == "2"
									If (Esc # 1) .Or. (Esc == 1 .And. SRV->RV_ADIANTA == "S")
										fSomaPdRec("D",SRD->RD_PD,Eval(cNroHoras),SRD->RD_VALOR,SRD->RD_ROTEIR)
										TOTDESC += Srd->Rd_Valor
									Endif
								Elseif SRV->RV_TIPOCOD == "3"
									If (Esc # 1) .Or. (Esc == 1 .And. SRV->RV_ADIANTA == "S")
										fSomaPdRec("B",SRD->RD_PD,Eval(cNroHoras),SRD->RD_VALOR,SRD->RD_ROTEIR)
									Endif
								Endif
							Endif
							If ESC = 1
	...
	If nTipRel == 1 .and. !( lTerminal )
		...
		IF !( lTerminal )
			...
			If Vez = 0  .and. aReturn[5] # 1   //JANIA 
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
AcDescFunc := Alltrim(Posicione("SRJ", 1, xFilial("SRJ") + alltrim(cCodFunc), "RJ_DESC")) // JANIA - 14/04/10
...
Local nContr  := 0
Local nContrT := 0
Local nLinhas := 16 //Numero de Linhas do Miolo do Recibo - 19 - JANIA

nLim := 0 //JANIA - 07/05/2013
...
For nConta = 1 To Len(aLanca)
	...
	If nContr = nLinhas .And. nContrT < Len(aLanca)
		nContr:=0
		//_oPrint:Say(_cont+0100, 0600, " *** CONTINUA *** ", _oFont14) 
		_oPrint:Say(nLim+40, 0600, " *** CONTINUA *** ", _oFont14) //JANIA - 07/05/2013
		_cont := 1550
		fCabecGrf()
		nLin := 450
	Endif
...
cDescFunc := Alltrim(Posicione("SRJ", 1, xFilial("SRJ") + ALLTRIM(SRA->RA_CODFUNC), "RJ_DESC")) //JANIA - 14/04/10

//Cabeçalho do holerite dados da empresa
_cont+=40                                                 //Margem Para Cabecalho
//Linha 1
_oPrint:Say(_cont+010, 0600, Desc_Fil, _oFont13) //Nome da Empresa  /_aEmp[1]
//Linha 2
...
_oPrint:Say(_cont+0345, 0690,  "C.B.O.: " + Alltrim(Posicione("SRJ",1,xFilial("SRJ") + SRA->RA_CODFUNC, "RJ_CODCBO") ), _oFont8)//Campo descricao da Funcao
...
_oPrint:Say(_cont+0400, 0300, "Descrição"    , _oFont8b)
...
_oPrint:Say(_cont+0400, 1560, "Vencimentos", _oFont8b)
//Divisao Linha e Titulo Descontos
_oPrint:Line(_cont+0390, 1800, _cont+1380, 1800)
_oPrint:Say(_cont+0400, 2090, "Descontos"   , _oFont8b)
...
Local	cString := Transform(aLanca[nConta,5],cPict2)
Local 	nCol := If(aLanca[nConta,1]="P",1750,If(aLanca[nConta,1]="D",2250,27))

_oPrint:Say(_cont+nLin, 0115, aLanca[nConta,2], _oFont8) //Codigo da Versao
_oPrint:Say(_cont+nLin, 0300, aLanca[nConta,3], _oFont8) //Descrição da verba
If aLanca[nConta,1] # "B"        // So Imprime se nao for base
	_oPrint:Say(_cont+nLin, 1250, TRANSFORM(aLanca[nConta,4],"999.99"), _oFont8,,,,1)
Endif
_oPrint:Say(_cont+nLin, nCol, cString, _oFont8,,,,1) //Nome da Empresa CONTUFLEX
nLin += 40
nLim := _cont+nLin  //JANIA - 07/05/2013
...
_oPrint:Say(_cont+1090, 1455, "Total de Vencimentos", _oFont8b)
_oPrint:Say(_cont+1090, 1970, "Total de Descontos"   , _oFont8b)
...
_oPrint:Say(_cont+1295, 1580, "Base Cálc. IRRF"  , _oFont8b)
...
//Impressao dos Totais Do campo Vencimentos
	...
	_oPrint:Say(_cont+1130, 1750,TRANSFORM(TOTVENC,cPict1), _oFont8,,,,1)
/*
If TOTVENC <= 9.99
	_oPrint:Say(_cont+1130, 1750,TRANSFORM(TOTVENC,cPict1), _oFont8,,,,1)
Endif
...
If TOTVENC <= 99.99 .and. TOTVENC > 9.99
	_oPrint:Say(_cont+1130, 1650,TRANSFORM(TOTVENC,cPict1), _oFont8,,,,1)
Endif

If TOTVENC <= 999.99 .and. TOTVENC > 99.99
	_oPrint:Say(_cont+1130, 1640,TRANSFORM(TOTVENC,cPict1), _oFont8,,,,1)
Endif

If TOTVENC <= 999999.99 .and. TOTVENC > 999.99
	_oPrint:Say(_cont+1130, 1630,TRANSFORM(TOTVENC,cPict1), _oFont8,,,,1)
Endif
*/
//Impressao dos Totais Do campo Descontos

_oPrint:Say(_cont+1130, 2250,TRANSFORM(TOTDESC,cPict1), _oFont8,,,,1)
/*
If TOTDESC <= 9.99
...
_oPrint:Say(_cont+1205, 2250,TRANSFORM((TOTVENC-TOTDESC),cPict1), _oFont8,,,,1)
...
Else
	nValSal := SRA->RA_SALARIO
EndIf
_oPrint:Say(_cont+1325, 0300, Transform(nValSal,cPict2) , _oFont8,,,,1)

If Esc = 1  // Bases de Adiantamento
	If cBaseAux = "S" .And. nBaseIr # 0
		_oPrint:Say(_cont+1325, 1780, TRANSFORM(nBaseIr,cPict1) , _oFont8,,,,1)
	Endif
ElseIf Esc = 2 .Or. Esc = 4  // Bases de Folha e 13o. 2o.Parc.
	If cBaseAux = "S"
		_oPrint:Say(_cont+1325, 0710,Transform(nAteLim,cPict1) , _oFont8,,,,1)
		If nBaseFgts # 0
			_oPrint:Say(_cont+1325, 1220, TRANSFORM(nBaseFgts,cPict1) , _oFont8,,,,1)
		Endif
		If nFgts # 0
			_oPrint:Say(_cont+1325, 1510,TRANSFORM(nFgts,cPict2) , _oFont8,,,,1)
		Endif
		If nBaseIr # 0
			_oPrint:Say(_cont+1325, 1780,TRANSFORM(nBaseIr,cPict1) , _oFont8,,,,1)
		Endif
