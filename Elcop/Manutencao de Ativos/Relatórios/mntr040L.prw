...
static Function R040Imp(lEnd,wnRel,titulo,tamanho)
	...
	BeginSql Alias cAliasQry
		...
		SELECT TQN.TQN_TANQUE, TQN.TQN_CODCOM, TQN.TQN_DTABAS, TQN.TQN_HRABAS, TQN.TQN_QUANT,
...
//		If (cComb != (cAliasQry)->TQN_CODCOM) .Or. (!Empty((cAliasQry)->TQN_TANQUE) .And. cTanque <> (cAliasQry)->TQN_TANQUE)
			...
			If !Empty((cAliasQry)->T6_MEDIA1)
				...
				cCabec1  := STR0028+Space(8)+STR0029+Space(10)+STR0051+Space(8)+STR0031+Space(7)+STR0052+Space(09)+STR0033+Space(20)+; //"Data"###"Hora"###"Contador"###"Realizado"###"Incremento"##"Abastecido"
		...
		If !Empty((cAliasQry)->T6_MEDIA1)
			...
			@ Li,070+3-Len(cTipCont) Psay (cAliasQry)->TQN_QUANT Picture "@E 999,999,999.999"
			@ Li,086+1-Len(cUM) Psay cUM
			@ Li,091+6-Len(cUnMedia) Psay nMediaR Picture "@E 999,999,999.99"
			@ Li,106+6-Len(cUnMedia) Psay cUnMedia
			@ Li,113 Psay (cAliasQry)->TQN_VALTOT Picture "@E 999,999,999,999,999."+Replicate('9',TAMSX3("TQN_VALTOT")[2])
			@ Li,138 Psay (cAliasQry)->TQN_VALUNI Picture "@E 999,999,999."+Replicate('9',TAMSX3("TQN_VALUNI")[2])
			@ Li,158 Psay STOD((cAliasQry)->TQN_DTDIGI)
			@ Li,168 Psay nLtAcu Picture "@E 999,999,999.999"
			NgSomaLi(58)
		...
		Else
			...
			@ Li,079 Psay nKmRod/(cAliasQry)->TQN_QUANT Picture "@E 999,999,999.99"
		...
		If cUndMed = '1'
			cTMediaR := nTKmRod/nTLitro
		ElseIf cUndMed = '2'
			cTMediaR := nTLitro/nTKmRod
		EndIf
		...
		If cFrota != (cAliasQry)->T9_CODBEM .OR. cTanque != (cAliasQry)->TQN_TANQUE .Or. cComb != (cAliasQry)->TQN_CODCOM
			If !Empty( (cAliasQry)->T6_MEDIA1 )
				...
				@ Li,065+3-Len(cTipCont) Psay cTipCont
				@ Li,070+3-Len(cTipCont) Psay nTLitro Picture "@E 999,999,999.999"
				@ Li,086+1-Len(cUM) Psay cUM
				@ Li,91+6-Len(cUnMedia) Psay cTMediaR Picture "@E 999,999,999.99"
				@ Li,106+6-Len(cUnMedia) Psay cUnMedia
				@ Li,114 Psay nTValor Picture "@E 99,999,999,999,999."+Replicate('9',TAMSX3("TQN_VALTOT")[2])
			Else
				NgSomaLi(58)
				@ Li,000 Psay STR0044+AllTrim(cFrota) + " / " + STR0045 + cTanque  //"Total Veiculo: "###"Tanque: "
				@ Li,049 Psay nTKmRod Picture "@E 999,999,999"
				@ Li,062 Psay nTLitro Picture "@E 999,999,999.999"
				@ Li,079 Psay nTKmRod/nTLitro Picture "@E 999,999,999.99"
...
/*/
//---------------------------------------------------------------------
static Function M040KMANT(dData,cHora,cFilBem,cCodBem)
	...
	If !Eof() .And. !Bof()
		If cCodBem == (cAliasQry)->TQN_FROTA
			...
			nKmRod := IIf(nKmAnt != 0,nKmAtual - nKmAnt,fBuscaIncr(dData,cHora,nKmAtual))
...
/*/
//---------------------------------------------------------------------
static Function M040LTACU(dData,cHora,cFilBem,cCodBem)
