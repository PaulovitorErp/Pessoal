...
TRCell():New(oSection1,"NOTLIC" 		,"   ","Licença=LI","@e 999.99",15 /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,"NOTALV" 		,"   ","Alvarás=AL","@e 999.99",15 /*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
...
TRCell():New(oSection1,"TC" 			,"   ","Trato Com=TC","@e 999",13/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
...
cQuery := "SELECT 	SA2.A2_COD, SA2.A2_LOJA, SA2.A2_NOME, SA2.A2_CGC, SA2.A2_XNOTDOC AS DOC, SA2.A2_XNOTLIC AS NOTALIC, "
cQuery += "	SA2.A2_XNOTALV AS NOTAALV, SA2.A2_XNOTDOC, SA2.A2_XNOTCOM NOTCOM, "
...
While !oReport:Cancel() .And. !(cAliasSD1)->(Eof())
	...
	nAE			:= 0
		...
		nNotaDoc	:= 0
		nLIC		:= (cAliasSD1)->NOTALIC
		nALV 		:= (cAliasSD1)->NOTAALV
		nCom		:= (cAliasSD1)->NOTCOM
		nDOC		:= (cAliasSD1)->DOC
	...
	If cXTIPDOC = "1"
		if nQtdeEnt > 0 
			nNotaDoc := (nLic + nALV + nDOC ) / 3
		else
			nNotaDoc := (nLic + nALV + nCOM + nDOC )  / 4
		endif

	elseif cXTIPDOC = "2"
			if nQtdeEnt > 0 
				nNotaDoc := (nALV + nDOC) / 2
			else
				nNotaDoc := (nALV + nDOC + nCOM ) / 3
			endif
	else
		if nQtdeEnt > 0 
			nNotaDoc := nDOC
		else
			nNotaDoc := (nDoc + nCom) / 2 
		endif
	...
	oSection1:Cell("NOTLIC"):SetValue(nLic)
	oSection1:Cell("NOTLIC"):SetAlign("CENTER")

	oSection1:Cell("NOTALV"):SetValue(nALV)
	oSection1:Cell("NOTALV"):SetAlign("CENTER")
	...
	if nQtdeEnt > 0 
		...
		if cXTIPDOC $ "1-2"
			nIQF := Round( 0.2 * ((nQtdeEnt- nQtdeAtr)/if(nQtdeEnt=0,1,nQtdeEnt))*100 + 0.2 * (nNotaDoc) + 0.3 * nQP + 0.2 * nAE + 0.1 * nCom, 0)
		else
			nIQF := Round( 0.2 * ((nQtdeEnt- nQtdeAtr)/if(nQtdeEnt=0,1,nQtdeEnt))*100 + 0.3 * (nNotaDoc) + 0.3 * nQP + 0.1 * nAE+ 0.1 * nCom , 0)
		endif
	else
		nIQF := nNotaDoc
	endif
	...
	oSection1:Cell("TC"):SetValue(nCom)
	oSection1:Cell("TC"):SetAlign("CENTER")
	...
	if MV_PAR04 = 2  // 1=Não e 2=Sim
		...
		if SA2->(DbSeek(xFilial("SA2")+cChaveFor))
			...
			If MV_PAR01 >= SA2->A2_XDTAIQF 
				SA2->(recLock("SA2",.F.))
				SA2->A2_XIQFMED := (SA2->A2_XIQF + nIQF) / 2  // iqf médio
				SA2->A2_XIQF 	:= nIQF
	...
	//Verifica a Condição de Pagamento com mais Utilização
	//Inicio
	cCondMed := fBusCP(cChaveFor)

	SA2->(recLock("SA2",.F.))
	SA2->A2_XCONDPG := cCondMed
	SA2->(msUnLock())		
...
static function  fBusCP(cCodFor)
	Local cCP 		:= ""
	Local cQuery 	:= ""
	Local cAliasTOP := GetNextAlias()
	Local aAreas	:= GetArea()

	cQuery := "SELECT TOP 1 F1_COND, COUNT(F1_FILIAL) QTDE "
	cQuery += "FROM "+retSqlName("SF1") + " SF1 "
	cQuery += "WHERE D_E_L_E_T_ = ' '  "
	cQuery += "AND F1_FORNECE = '"+substr(cCodFor,1,6)+"' AND F1_LOJA = '"+substr(cCodfor,7,2)+"' "
	cQuery += "GROUP BY F1_COND "
	cQuery += "ORDER BY QTDE DESC "
	cQuery := ChangeQuery(cQuery)
	dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasTOP,.T.,.T.)

	(cAliasTOP)->(DbGoTop())
	cCP := (cAliasTOP)->F1_COND
