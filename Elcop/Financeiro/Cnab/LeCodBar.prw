...
DEFINE MSDIALOG _oDlgBar TITLE "Codigo de Barras" FROM 178,181 TO 630,775 PIXEL
...
@ 145,003 TO 210,297 LABEL " Observa็ใo Do Titulo " PIXEL OF _oDlgBar
...
@ 155,007 Get oMemo1 Var cMsg MEMO Size 281,050 /*READONLY*/ PIXEL OF _oDlgBar 
...
If Alltrim(ReadVar())= "M->E2_CODBAR"
	If Empty(M->E2_CODBAR)
		...
		if !empty(M->E2_VALOR)
			if strzero(val(str(int(M->E2_VALOR))+iif(AT(".",str(M->E2_VALOR)) > 0,;
			iif(len(SUBSTR(str(M->E2_VALOR), AT(".",str(M->E2_VALOR)) +1,2)) == 1,SUBSTR(str(M->E2_VALOR);
			, AT(".",str(M->E2_VALOR)) +1,2)+"0",SUBSTR(str(M->E2_VALOR), AT(".",str(M->E2_VALOR)) +1,2)),"00")),10) <> substr(cCodbarras,10,10)
