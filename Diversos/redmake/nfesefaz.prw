...
//TBC-GO: Personalizacao: Crio Array para complemento mensagem monofasica
Private aMonof61	:= {} // Msg infCPL para ICMS Monofasico
...
Private lServDf		:= .F.
// Customização - José Orfeu - 11/03/2024 - Ultima alteração 12/03/2024
Private	dC5FecEnt   := dDataBase

//TBC-GO Personalizacao
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				#IFDEF TOP
					...
					//Verifica se existe Template DCL   //Personalizacao: Alteracao Fabricio para que nao excecute templete na grupo 01 Rodoviario
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) .AND. cEmpAnt $ GetNewPar( "ZZ_LDRTRR", "02/03" ) //Tratativa para Grupo de Repasse de Combustiveis
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If lNfCup
							...
							//TBC-GO Personalizacao: Adicionado +(cAliasSD2)->D2_COD+cNumitem
							If Dbseek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEM)
								cD2Tes	:= SD2->D2_TES
							EndIf	
						...
						*/
						...
						//Verifica se existe Template DCL  //Personalizacao: Alteracao Fabricio para que nao excecute templete na grupo 01 Rodoviario
	      				IF (ExistTemplate("PROCMSG")) .AND. cEmpAnt $ GetNewPar( "ZZ_LDRTRR", "02/03" )
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								Case AllTrim(CD2->CD2_IMP) == "ICM"
                                	...
                                	//Personalizacao Decio para tratara templete de distribuicao de combustivel
									IF  cEmpAnt $ GetNewPar( "ZZ_LDRTRR", "02/03" ) //Tratativa para Grupo de Repasse de Combustiveis
...
IF lPe01Nfe     
	...
	//-- TBC-GO Personalizacao: Acrescentado o vetor aComb para manipular os dados complementares de combustiveis, e outras variaveis e arrays
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aProcRef,aMed,aLote,;
				aComb,cIndPres,cModFrete,aFat,aTotal,aCST,aICMS,cVerAmb,aPIS,aCOFINS,aICMSMONO}
...
If !Empty(aNota)
	...
	if lMonof61
		...
		//TBC-GO Personalizacao: Complemento mensagem monofasico
		for nX := 1 to len(aMonof61)
			If aMonof61[nX][3] > 0
				cMensCpl += " Produto "+alltrim(aMonof61[nX][04])+", BC "+cValToChar(aMonof61[nX][01])+" (em litros), Alíquota: R$ "+Alltrim(str(aMonof61[nX][02],15,4))+", ICMS mono: R$ "+Alltrim(str(aMonof61[nX][03],15,2))+";"
			EndIf
		next nX
	endIf
...
// Customização - José Orfeu - 11/03/2024 - Ultima alteração 12/03/2024
// Imprime a data de entrega - dC5FecEnt = SC5->C5_FECENT
If !Empty(dC5FecEnt)
	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(dC5FecEnt)+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
Else
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	*/
	...
	if !lBrinde
		//TBC-GO Personalizacao:
		aMailDest := StrToKArr(AllTrim(aDest[16]),";") 
		if len(aMailDest)>0
			cString += NfeTag('<EMAIL>',AllTrim(aMailDest[1])+cMailTrans)
...
//TBC-GO Personalizacao: Arredondamento
//cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],23,10))+'</vUnTrib>'	
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(Round(aProd[10]/aProd[12],10),23,10))+'</vUnTrib>'	
...
If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
	...
	if isNT230004() .and. len(allTrim(aDI[39][3])) == 11
		cString += NfeTag('<CPF>',ConvType(aDI[39][3],11))
	else
...
Elseif Len(aDI)>0
	...
	if isNT230004() .and. len(allTrim(aDI[21][3])) == 11
		cString += NfeTag('<CPF>',ConvType(aDI[21][3],11))
	else
...
If Len(aComb) > 0  .And. !Empty(aComb[01])
	...
	*/
	If Len(aComb) > 4 .and. !Empty(aComb[09])
		...
		//TBC-GO - colocado 3 casas decimais no encerrante inicial e final
		cString += '<vEncIni>'+ConvType(aComb[12],15,3)+'</vEncIni>'
		cString += '<vEncFin>'+ConvType(aComb[13],15,3)+'</vEncFin>'
		cString += '</encerrante>'		
	EndIf
	If Len(aComb) > 23
...
If  !lIssQn    
	...
	ElseIf aCST[01] $ "02-15-53-61"
		...
		If(aCST[1] $ '61')
			...
			//TBC-GO Personalizacao: tratativa para complemento da mensagem monofasico
			if len(aICMSMono) >= 7
				nX := aScan(aMonof61, {|x| x[4] == aProd[02] .AND. x[2] == aICMSMono[06]})
				if nX == 0
					aadd(aMonof61, {aICMSMono[05],aICMSMono[06],aICMSMono[07],aProd[02]})
				else //somo somente base e valor
					aMonof61[nX][1] += aICMSMono[05]
					aMonof61[nX][3] += aICMSMono[07]
				endif
			endif
...
If Len( aNfVinc ) > 0 //Nota de espécie NFE ou NCE ou CTE vinculada
	For nZ := 1  to Len( aNfVinc )
		If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
			...
			//TBC-GO Personalizacao: Adicionado placa
			If !Empty(aNfVinc[nZ][6]) .and. "NFCE" == UPPER(Alltrim(aNfVinc[nZ][6]))
				cNfVinc := ( Alltrim(aNfVinc[nZ][2] + " " + aNfVinc[nZ][3] )+ " " + StrZero( Day( aNfVinc[nZ][1] ), 2 ) + "-" + StrZero( Month( aNfVinc[nZ][1] ), 2 ) + "-" + StrZero( Year( aNfVinc[nZ][1] ), 4 ) ;
							+ " " + Alltrim(Posicione("SL1",2,xFilial("SL1")+aNfVinc[nZ][2]+aNfVinc[nZ][3],"L1_PLACA")) ;
							+ ", " )
			Else
				cNfVinc := ( Alltrim(aNfVinc[nZ][2] + " " + aNfVinc[nZ][3] )+ " " + StrZero( Day( aNfVinc[nZ][1] ), 2 ) + "-" + StrZero( Month( aNfVinc[nZ][1] ), 2 ) + "-" + StrZero( Year( aNfVinc[nZ][1] ), 4 ) + ", " )
			EndIf
...
/*
//TBC-GO Personalizacao: Converte caracteres especiais
Manual de Orientação Contribuinte da NF-e, aonde condiz o tratamento dos caracteres especiais:
...
Endereço e Informação Adicional de uma NF-e. 
Para resolver esses casos, é recomendável a substituição do caractere que causa o problema.
...
IF len(aDetPag) > 0  
	...
	For nX := 1 To Len(aDetPag)
		...
		//TBC-GO Personalizacao: Rejeição 441: Descrição do pagamento obrigatória para meio de pagamento 99- outros		
		if aDetPag[nX][1] == "99" .And. len(aDetPag[nX]) >= 9
			cString +='<xPag>' + aDetPag[nX][9] + '</xPag>'
		elseif aDetPag[nX][1] == "99"
			cString +='<xPag>' + 'PAGAMENTO POSTO INTELIGENTE' + '</xPag>'
		endIf
		...
		endIf
		if isNT230004() .and. Len(aDetPag[nX]) > 10 .and. len(aDetPag[nX][11]) > 0 .and. !empty(aDetPag[nX][11][1])
...
Funcao que determina se a NT 2023.004 ja esta em operacao.
...
if lNT23004 == nil
	lNT23004 := date() >= cTod(superGetMV("MV_NT23004",nil,"01/04/2024"))
endIf
return lNT23004
