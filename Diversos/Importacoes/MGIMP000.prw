...
Static Function Pag3Inf(oPanel)
	...
	@ 120 , 020 BUTTON oBtnImp PROMPT "Visualizar Log" SIZE 070, 015 OF oPanel PIXEL ACTION (ShellExecute("Open",cDirLog + cNomeArq + ".csv"," ","C:\",1))
...
Static Function PartSup(oPanel)
	...
	@ 025 , 070 SAY oSay2 PROMPT "Siga atentamente os passos para realizar a importação de dados.";
...
Static Function VldPag2()
	...
	If lRet
		...
		nHdlLog := MsfCreate(cDirLog + cNomeArq + ".csv",0) //nHdlLog := MsfCreate(strtran(cDirLog + cNomeArq,"/","-") + ".txt",0)
		If nHdlLog < 0
			lRet := .F.
...
Static Function ProcImp(oSay)
	...
	If nHdlLog > 0
		...
		Case SubStr(cEntidade,1,2) == "01" // Importa cadastro de clientes - SA1.
			fWrite(nHdlLog,"CNPJ/CPF;NOME;RESULTADO;")
			fWrite(nHdlLog,cPulaLinha)
			U_MGIMP001(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)

		Case SubStr(cEntidade,1,2) == "02" // Importa cadastro de fornecedores - SA2.
			fWrite(nHdlLog,"CNPJ/CPF;NOME;RESULTADO;")
			fWrite(nHdlLog,cPulaLinha)
			U_MGIMP002(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)

		Case SubStr(cEntidade,1,2) == "03" // Importa cadastro de produtos - SB1.
			...
			U_MGIMP003(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
			fWrite(nHdlLog,Replicate("-",44))
			fWrite(nHdlLog,cPulaLinha)
		...
		Case SubStr(cEntidade,1,2) == "04" // Importa contas a pagar - SE2.
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP004(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
		...
		Case SubStr(cEntidade,1,2) == "05" // Importa contas a receber - SE1.
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP005(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
		...
		Case SubStr(cEntidade,1,2) == "06" // Importa Saldos em Estoque SB9.
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP006(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
		...
		Case SubStr(cEntidade,1,2) == "07" // Contratos
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP007(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
		...
		Case SubStr(cEntidade,1,2) == "08" // Baixas a Receber
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP008(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
		...
		Case SubStr(cEntidade,1,2) == "09" // Baixas a Pagar
			fWrite(nHdlLog,"#########  IMPORTACAO DE DADOS #############")
			fWrite(nHdlLog,cPulaLinha)
			fWrite(nHdlLog," >> Data inicio: " + DTOC(Date()))
			fWrite(nHdlLog,cPulaLinha)
			...
			U_MGIMP009(oSay,cArquivo,@nHdlLog,@nObjProc,@nSucesso)
