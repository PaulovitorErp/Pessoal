...
Static nPosNsuTef, nPosDocTef, nPosCartAu, nPosCGC, nPosObsFat
...
/***********************/
	...
	Local oMenuNf, oIt1Nf, oIt2Nf, oIt3Nf
	Local oMenuDiv, oIt1Div, oIt2Div, oIt3Div, oIt4Div, oIt5Div, oIt6Div

	Local nPosTit
	Local lLogExc	:= SuperGetMv("MV_XMOTEXF",.F.,.F.) .AND. ChkFile("U0J")
	...
	Private oGet23, oGet24, oGet25
	Private oSit
	Private oCheckBox1, oCheckBox2, oCheckBox3, /*oCheckBox4,*/ oCheckBox5, oCheckBox6, oCheckBox7, oCheckBox8, oCheckBox9

	Private cGet4		:= Space(len(cFilAnt))
	Private cGet5		:= Space(25)
	Private cGet6		:= Space(200)
	Private cGet7		:= Space(200)
	Private cGet8		:= Space(200)
	Private cGet9		:= Space(200)
	Private cGet10		:= Space(200)
	Private cGet11		:= Space(200)
	Private cGet12		:= Space(200)
	Private dGet13		:= CToD("")
	Private dGet14		:= CToD("")
	Private dGet15		:= CToD("")
	Private dGet16		:= CToD("")
	Private cGet17		:= Space(200)
	Private cGet18		:= Space(200)
	Private dGet19		:= CToD("")
	Private dGet20		:= CToD("")
	Private cGet21 		:= Space(TamSX3("E1_NUM")[1])
	Private cGet22 		:= Space(TamSX3("E1_NUM")[1])
	Private cGet23		:= IIF(!lFatConv,Space(TamSX3("DA3_PLACA")[1]),"")
	Private dGet24		:= dDataBase
	Private cGet25		:= ""

	Private lCheckBox1	:= .T.
	Private lCheckBox2	:= .F.
	Private lCheckBox3	:= .F.
	Private lCheckBox4	:= .F.
	Private lCheckBox5	:= .F.
	Private lCheckBox6 	:= .F.
	Private lCheckBox7 	:= .T.
	Private lCheckBox8 	:= .T. //IIF(!lMarajo,.F.,.T.)
	Private lCheckBox9 	:= .F.
	Private lCB10 		:= .F.
	...
	Private oButton27, oButton28, oButton29
...
//Pasta Filtro
	...
	@ 137, 515 SAY oSay37 PROMPT "Obs. Fatura" SIZE 040, 007 OF oFolderFat:aDialogs[1] COLORS 0, 16777215 PIXEL
	@ 145, 515 GET oGet25 VAR cGet25 MEMO SIZE 100, 040 OF oFolderFat:aDialogs[1] COLORS 0, 16777215 PIXEL VALID (cGet25 := Upper(cGet25))
...
// Adiciona itens no Menu Fatura
	oIt1Nf := TMenuItem():New(oDlgT017,"Gerar",,,,{||GeraNFe()},,,,,,,,,.T.)
	oIt2Nf := TMenuItem():New(oDlgT017,"Estornar",,,,{||EstNfe()} ,,,,,,,,,.T.)
	oIt3Nf := TMenuItem():New(oDlgT017,"Imprimir DANFE",,,,{||ImpNfe()} ,,,,,,,,,.T.)
	oIt4Nf := TMenuItem():New(oDlgT017,"NF-e Sefaz",,,,{|| SPEDNFE() },,,,,,,,,.T.)
...
// Adiciona itens no Menu Diversos
	...
	oIt7Div := TMenuItem():New(oDlgT017,"Log Exclusão Faturas",,,,{|| U_TRETR018() },,,,,,,,,.T.)
...
// Legenda
	...
	@ aPosObj[2,1] - 50, aPosObj[2,4] - 77 BUTTON oButton27 PROMPT "Ver Prévia da Fatura" SIZE 070,013 OF oFolderFat:aDialogs[2] ACTION U_TRETE047(aSizeAut,aPosObj) PIXEL
...
User Function TRE017CP(nOpc, cVarPos)
	...
	if aCfgCampos == Nil
		...
		aCfgCampos := {aCabec, aSizes, aLinEmpty}
	else
		aCabec 		:= aCfgCampos[1]
		aSizes 		:= aCfgCampos[2]
...
/****************************/
	...
	Local cFuncBol		:= SuperGetMv("MV_XFUNBOL",.F.,"TRETR009")
	Local cBlFuncBol
...
//Verifica se é fatura de fatura
	For nI := 1 To Len(aReg)
		...
		If aReg[nI][1] == .T. // Título selecionado
	...
	If lAux
		...
		If MsgYesNo("Haverá o faturamento dos registros selecionados, deseja continuar?")
			...
			If !lFatFat
				...
				For nI := 1 To Len(aCliente)
					...
					For nX := 1 To Len(aTmpProd)
						...
						oSay:cCaption := "Gerando fatura "+cValToChar(nX)+" de "+cValToChar(Len(aTmpProd))+""
						ProcessMessages()

						aAuxFat := U_TRETE016(aTmpProd[nX][1],aCliente[nI][1],aCliente[nI][2],3,,,,,,,,,,,,,lFatFat .AND. !lDifCartao)
							...
							if Alltrim(aTmpProd[nX][1][1][nPosTipo]) $ "CC/CD/PX/" //se a fatura é de cartao, chamo baixa
								...
								If lMarajo
									...
									If !lFatConv // Diferente de conveniência
										...
										If !U_TRETE020(,cFilAnt,{{aAuxFat[1][1],aAuxFat[1][2],aAuxFat[1][3],aAuxFat[1][4],aAuxFat[1][5],aAuxFat[1][6]}},.T.,,,,cGet25)

											//Conout(" >> Arquivo com tamanho zerado, serão realizadas "+cValToChar(nNroTen)+" tentativas de geracao.")

											While nContAux < nNroTen

												nContAux++

												//Conout(" >> "+StrZero(nContAux,2)+" tentativa de geracao do arquivo")
								...
								Else
									//Gera arquivo PDF da Fatura
									U_TRETE020(,cFilAnt,{{aAuxFat[1][1],aAuxFat[1][2],aAuxFat[1][3],aAuxFat[1][4],aAuxFat[1][5],aAuxFat[1][6]}},.T.,,,/*@__aArqPDF*/,cGet25)
								Endif
							endif
						Endif
					Next nX
					...
					For nI := 1 To Len(aFatura)
						...
						If nImpFatur == 0
							...
							If MsgYesNo("Deseja imprimir a fatura?")
								...
								If lMarajo
									U_TRETE020(oSay,cFilAnt,{{aFatura[nI][1],aFatura[nI][2],aFatura[nI][3],aFatura[nI][4],aFatura[nI][5],aFatura[nI][6]}},.F.,.T.,,,cGet25)
								Else
									U_TRETE020(oSay,cFilAnt,{{aFatura[nI][1],aFatura[nI][2],aFatura[nI][3],aFatura[nI][4],aFatura[nI][5],aFatura[nI][6]}},.F.,.F.,,,cGet25)
								Endif
							Endif
							...
							If nImpBol == 0
								...
								If MsgYesNo("Deseja imprimir o boleto?")
									...
									If SE1->(DbSeek(xFilial("SE1")+aFatura[nI][2]+aFatura[nI][3]+"FAT"+aFatura[nI][1]))
										...
										cBlFuncBol := "{|oSay| U_"+cFuncBol+"(aBoleto,,,,.F.)}"
				...
				If Len(aFatura) == 1 .AND. lDifCartao
					If !lMarajo
						If lEnvArqs
			...
			Else // Fatura de Fatura
				...
				aFatura := U_TRETE016(aTit,cCli,cLojaCli,4)
				...
				If Len(aFatura) > 0
					...
					If lMarajo
						...
						If !lFatConv // Diferente de conveniência
							...
							If !U_TRETE020(,cFilAnt,{{aFatura[1][1],aFatura[1][2],aFatura[1][3],aFatura[1][4],aFatura[1][5],aFatura[1][6]}},.T.,,,,cGet25)
					...
					Else
						//Gera arquivo PDF da Fatura
						U_TRETE020(,cFilAnt,{{aFatura[1][1],aFatura[1][2],aFatura[1][3],aFatura[1][4],aFatura[1][5],aFatura[1][6]}},.T.,,,/*@__aArqPDF*/,cGet25)
					Endif
					...
					If nImpFatur == 0
						...
						If MsgYesNo("Deseja imprimir a fatura?")
							...
							If lMarajo
								U_TRETE020(oSay,cFilAnt,{{aFatura[1][1],aFatura[1][2],aFatura[1][3],aFatura[1][4],aFatura[1][5],aFatura[1][6]}},.F.,.T.,,,cGet25)
							Else
								U_TRETE020(oSay,cFilAnt,{{aFatura[1][1],aFatura[1][2],aFatura[1][3],aFatura[1][4],aFatura[1][5],aFatura[1][6]}},.F.,.F.,,,cGet25)
							EndIf
						Endif
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	If Len(aTit) > 0
		...
		If MsgYesNo("Haverá o faturamento dos registros selecionados, deseja continuar?")
			...
			aFatura := U_TRETE016(aTit,cCli,cLojaCli,3,,lFatFat,.T.,,,,,,,,,,lFatFat .AND. !lDifCartao)

			// Aviso ao usuário
			If Len(aFatura) > 0

				if lHasCartao
					if MsgYesNo("Fatura "+Alltrim(aTit[1][nPosNome])+" gerada! Deseja baixar a fatura?")
						SE1->(DbGoTop())
						SE1->(DbSetOrder(2)) // E1_FILIAL+E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO
						If SE1->(DbSeek(xFilial("SE1")+aFatura[1][2]+aFatura[1][3]+"FAT"+aFatura[1][1]))
							Liq(SE1->(Recno()), .T.)
				...
				MsgInfo("Fatura processada com sucesso.","Atenção")
			Else
				MsgInfo("Nenhuma Fatura gerada.","Atenção")
			Endif
...
/*********************************/
	...
	If nCont > 0 .And. Len(aFat) > 0
		...
		If _nOpc == 1 // Direto na porta
			U_TRETE020(oSay,cFilAnt,aFat,.F.,.T.,,,cGet25)
			MsgInfo("Processamento finalizado.","Atenção")
		Else
			U_TRETE020(oSay,cFilAnt,aFat,,,,,cGet25)
		Endif
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		...
		For nI := 1 To Len(aReg)
			...
			If aReg[nI][1] == .T.
				...
				If nTp == 1 .Or. nTp == 2 // Gerar/Imprimir direto na porta
					...
					cBlFuncBol := "U_"+cFuncBol+"(aBoleto,aImp[1],,,lImprime)"
					&cBlFuncBol
				Else // Gerar/Imprimir em tela
					//U_TRETR009(aBoleto,,,,.F.)
					cBlFuncBol := "U_"+cFuncBol+"(aBoleto,,,,.F.)"
					&cBlFuncBol
				Endif
			Endif
		Next
	...
	Else // Função Faturar
		...
		// U88_FILIAL+U88_FORMAP+U88_CLIENT+U88_LOJA
		If U88->(DbSeek(xFilial("U88")+"FT"+Space(4)+_cCli+_cLojaCli))
		...
		aImp := GetImpWindows(.F.) // Busca a relação de impressoras da estação, onde a primeira da lista é a padrão
		If lMarajo
			//U_TRETR009(aBoleto,aImp[1],,,lImprime)
			cBlFuncBol := "U_"+cFuncBol+"(aBoleto,aImp[1],,,lImprime)"
			&cBlFuncBol
		Else
			...
			cBlFuncBol := "U_"+cFuncBol+"(aBoleto,,,.T.,.F.)"
			&cBlFuncBol
		Endif
	Endif
...
/***********************/
	...
	FWMsgRun(,{|oSay| U_TRETE033()},'Aguarde','Estornando Nota Fiscal...')
	Processa({|lEnd| Filtro(@lEnd)}, "Realizando a consulta...")
				...
				If nSaldo > 0
					...
					If _nOpc ==  1 .Or. _nOpc == 2 .Or. _nOpc == 3 .Or. _nOpc == 4
						...
						Else // Imprimir Contrato de Renegociação
							if lMarajo
								U_TRETR012(aReg)
							elseif ExistBlock("TR017CRN") //PE para impressão do contra de renegociação
...
Local lLogExc	:= SuperGetMv("MV_XMOTEXR",.F.,.F.) .AND. ChkFile("U0J")//parametro para renegociação
Local aDadosFat	:= {}
	...
	If Len(aTit) > 0
		...
		If MsgYesNo("Haverá o faturamento individual dos registros selecionados, deseja continuar?")
			...
			For nI := 1 To Len(aTit)
				...
				aFatura := U_TRETE016({aTit[nI]},cCli,cLojaCli,iif(lFatFat,4,3),,lFatFat)
				...
				If Len(aFatura) > 0
					...
					If U88->(DbSeek(xFilial("U88")+"FT"+Space(4)+aFatura[1][2]+aFatura[1][3]))
						If U88->U88_TPCOBR == "B" //Boleto Bancário
							//Gerar Boleto
							ImpBol(oSay,5,cFilAnt,aFatura[1][1],aFatura[1][2],aFatura[1][3],"FAT")
						Endif
					Endif

					/*If lMarajo
						U_GeraNFe(cFilAnt,"FT ","FAT",aFatura[1][1],aFatura[1][2],aFatura[1][3],,.F.)
					EndIf */
				EndIf
			Next nI
...
/***********************/
	...
	if lMarajo .OR. ExistBlock("TR017CRN")
...
/*****************************************/
	...
	Local _aAlcada		:= {}
	Local lAux
	Local lLogRen		:= SuperGetMv("MV_XMOTREN",.F.,.F.) .AND. ChkFile("U0J")//parametro para renegociação
	Local aDadosFat	:= {}
			...
			Begin Transaction
				...
				If !Empty(SE1->E1_NUMLIQ) // Liquidação
					...
					aFatura := U_TRETE016(;
						...
						"RENEG. "+SE1->E1_PREFIXO+"-"+SE1->E1_NUM+"/"+SE1->E1_PARCELA+"")
...
/*********************************/
	...
	cQry += CRLF + " FROM "+RetSqlName("SL1")+" (NOLOCK) SL1 "
	cQry += CRLF + " INNER JOIN " + RetSqlName("SL2") + " (NOLOCK) SL2 "
...
/*****************************************/
Static Function SelDanfes(cFil,cFat,aArqPDF)
/*****************************************/
	...
	cQry += CRLF + " AND SE1.E1_CLIENTE	= SF2.F2_CLIENTE"
	cQry += CRLF + " AND SE1.E1_LOJA		= SF2.F2_LOJA"
	cQry += CRLF + " AND SE1.E1_FATURA		= '"+cFat+"'"
...
/*************************************/
Static Function EnvPDFs(aArqPDF,cEmails)
/*************************************/
	...
	ElseIf nTipo == 2 //Parcelamento
		...
		For nI := 1 To Len(aParcelas)
			...
			If lRet
				...
				Else
					//imprime fatura
					U_TRETE020(,cFilAnt,{{cTit,SE1->E1_CLIENTE,SE1->E1_LOJA,SE1->E1_PREFIXO,SE1->E1_PARCELA,SE1->E1_TIPO}},.T.,,,,cGet25)
...
Static Function EnvEmailCli()
	...
	For nI := 1 To Len(aReg)
		If aReg[nI][1] == .T. .AND. Alltrim(aReg[nI][nPosTipo]) == "FT"

			if U_TRETE044(iif(nPosFilOri>0,aReg[nI][nPosFilOri],cFilAnt), {aReg[nI][nPosNumero],aReg[nI][nPosCliente],aReg[nI][nPosLoja],aReg[nI][nPosPrefixo],aReg[nI][nPosParcela],aReg[nI][nPosTipo]})
				lEnvOk := .T.
			endif
