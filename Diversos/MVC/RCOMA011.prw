...
Local oModel, oAux, oStruct
	...
	if MsgYesNo("Deseja cadastrar Turma vs Alunos?") 
		...
		aAdd( aCabZB1, { 'ZB1_FILIAL'	, xFilial("ZB1") 	} )
		aAdd( aCabZB1, { 'ZB1_CODIGO' 	, "006" 		 	} )
		aAdd( aCabZB1, { 'ZB1_DESCRI' 	, "TESTE" 			} )  
		aAdd( aCabZB1, { 'ZB1_PERIOD'	, "1" 				} )
		aAdd( aCabZB1, { 'ZB1_SITUAC' 	, "1"	 		 	} )
		
		aAdd( aAux1, { 'ZB6_FILIAL'		, xFilial("ZB6") 	} )
		aAdd( aAux1, { 'ZB6_CODTUR' 	, "006" 			} )
		aAdd( aAux1, { 'ZB6_RA' 		, "0001" 			} )
		
		aAdd( aItensZB6, aAux1 )
		
		aAux1 := {}
		
		aAdd( aAux1, { 'ZB6_FILIAL'		, xFilial("ZB6") 	} )
		aAdd( aAux1, { 'ZB6_CODTUR' 	, "006" 			} )
		aAdd( aAux1, { 'ZB6_RA' 		, "0002" 			} )
		
		aAdd( aItensZB6, aAux1 )
		
		aAdd( aAux2, { 'ZB7_FILIAL'	, xFilial("ZB7") 	} )
		aAdd( aAux2, { 'ZB7_CODTUR' , "006" 		 	} )
		aAdd( aAux2, { 'ZB7_RA' 	, "0001" 			} )  
		aAdd( aAux2, { 'ZB7_CODDIS'	, "001"				} )
		aAdd( aAux2, { 'ZB7_BIM' 	, "1" 				} )
		aAdd( aAux2, { 'ZB7_NOTA'	, 10 				} )			
		
		aAdd( aItensZB7, aAux2 )
		aAux2 := {}
		
		aAdd( aAux2, { 'ZB7_FILIAL'	, xFilial("ZB7") 	} )
		aAdd( aAux2, { 'ZB7_CODTUR' , "006" 		 	} )
		aAdd( aAux2, { 'ZB7_RA' 	, "0002" 			} )  
		aAdd( aAux2, { 'ZB7_CODDIS'	, "002"				} )
		aAdd( aAux2, { 'ZB7_BIM' 	, "1" 				} )
		aAdd( aAux2, { 'ZB7_NOTA'	, 10 				} )		
		...
		Grava( 'ZB1', 'ZB6', 'ZB7', aCabZB1, aItensZB6, aItensZB7 )
	
	endif

Return()

Static Function Grava( cMaster, cDetail, cDetail2, aCpoMaster, aCpoDetail, aCp2Detail )
...
//cMaster, cDetail, cDetail2, aCpoMaster, aCpoDetail, aCp2Detail 
//----------------------------------------------------------------------------------------------------------------------------------------------

If lRet
	// Intanciamos apenas a parte do modelo referente aos dados do item
	oAux     := oModel:GetModel( cDetail2 + 'DETAIL' )
	...
	For nI := 1 To Len( aCp2Detail )
		...
		For nJ := 1 To Len( aCp2Detail[nI] )

		// Verifica se os campos passados existem na estrutura de item
			If ( nPos := aScan( aAux, { |x| AllTrim( x[3] ) ==  AllTrim( aCp2Detail[nI][nJ][1] ) } ) ) > 0

				If !( lAux := oModel:SetValue( cDetail2 + 'DETAIL', aCp2Detail[nI][nJ][1], aCp2Detail[nI][nJ][2] ) )
